...
namespace pong2
{
    public static class InputBox
    {
        ...
        public static LoginPassword Profile(String login, String[] tops, int[] scores)
        {
            LoginPassword lp = new LoginPassword();
            CloseWindow();
            ...
            while (!WindowShouldClose())
            {
                ...
                {
                    DrawText($"{i}. {tops[i]} - {scores[i]} ", screenWidth / 2 +75, 60 + 30*i, 20, Color.DARKGRAY);
                }
                ...
                if (IsKeyPressed(KeyboardKey.KEY_ONE))
                {
                    ...
                    lp.field = 4;
                    lp.login = name;
                    lp.password = "not changed";
                    break;
                }
                if (IsKeyPressed(KeyboardKey.KEY_TWO))
                {
                    lp.field = 2;
                    lp.login = "not changed";
                    lp.password = "not changed";
                    break;
                }
                if (IsKeyPressed(KeyboardKey.KEY_THREE))
